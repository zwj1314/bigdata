...
function get_data(dataset) {
    ...
    if (dataset == '首投客户数_分时') {
        $.ajax({ type :"get", url :url, data :params, async :false, success :function (data, status) {
            ...
            option1 = get_echarts_option(['昨日', '今日'], '新客-分时', hor, [last_idx_val, idx_val], 'bar')
            option2 = get_echarts_option(['昨日', '今日'], '新客-累加', hor, [cumsum_last_idx_val, cums_sum_idx_val], 'line')
        ...
        $.ajax({ type :"get", url :url, data :params, async :false, success :function (data, status) {
            ...
            var cums_sum_idx_val = _.map(_.fill(_.map(idx_val, (value, index, array) => _.sum(_.slice(array, 0, index + 1))),null,now_hour),x=>x==null?null:x.toFixed(2))
