...
function get_data(dataset) {
    ...
    if (dataset == '用户注册-分时') {
        ...
        $.ajax({ type :"get", url :url, data :params, async :false, success :function (data, status) {
                var hor = _.values(data)[0].map(x => x['HOR']);
                var tiexinzhitou = _.values(data)[0].map(x => x['tiexinzhitou']);
                var tiexinzhitouxinshou = _.values(data)[0].map(x => x['tiexinzhitouxinshou']);
                var yueyueshengzhitou = _.values(data)[0].map(x => x['yueyueshengzhitou']);


                var cums_sum_tiexinzhitou = _.fill(_.map(tiexinzhitou, (value, index, array) => _.sum(_.slice(array, 0, index + 1))),null,now_hour)
                var cums_sum_tiexinzhitouxinshou = _.fill(_.map(tiexinzhitouxinshou, (value, index, array) => _.sum(_.slice(array, 0, index + 1))),null,now_hour)
                var cums_sum_yueyueshengzhitou = _.fill(_.map(yueyueshengzhitou, (value, index, array) => _.sum(_.slice(array, 0, index + 1))),null,now_hour)


                //console.log(cums_sum_idx_val);

                option6= get_echarts_option2([['贴心智投'], ['贴心在投-新手'], ['月月升智投']],  '各产品累计投资额-累加', hor, [cums_sum_tiexinzhitou, cums_sum_tiexinzhitouxinshou, cums_sum_yueyueshengzhitou], 'line')
        ...
        $.ajax({ type :"get", url :url, data :params, async :false, success :function (data, status) {
                ...
                option8= get_echarts_option2([['贴心智投'], ['贴心在投-新手'], ['月月升智投']], '各产品累计结算金额-累加', hor, [cums_sum_tiexinzhitou, cums_sum_tiexinzhitouxinshou, cums_sum_yueyueshengzhitou], 'line')
